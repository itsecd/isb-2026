"""Модуль графического интерфейса пользователя (GUI) для криптосистемы."""
...
class CryptoApp:
    ...
    def __init__(self, root: tk.Tk, config_path: str = 'settings.json') -> None:
        """Инициализирует графическое окно и загружает настройки.

        Args:
            root: Корневой объект окна tkinter.
            config_path: Путь к JSON-файлу с настройками.
        """
        self.root = root
        self.root.title("Гибридная система: Camellia + RSA")
    ...
    def _get_default_settings(self) -> Dict[str, str]:
        """Возвращает структуру настроек по умолчанию (константы блока)."""
        return {
    ...
    def _load_settings(self) -> Dict[str, str]:
        """Загружает настройки из JSON-файла, при ошибке возвращает дефолтные."""
        try:
            return file_io.read_json(self.config_path)
        except (FileNotFoundError, json.JSONDecodeError):
            return self._get_default_settings()

    def _create_widgets(self) -> None:
        """Создаёт элементы управления в окне приложения."""
        ...
        ttk.Label(frame_gen, text="Длина ключа Camellia:").grid(row=0, column=0, sticky="w", padx=5)
        ...
        btn_enc = ttk.Button(frame_ops, text="Зашифровать файл", width=25, command=self.handle_encryption)
        btn_enc.pack(pady=5)

        btn_dec = ttk.Button(frame_ops, text="Расшифровать файл", width=25, command=self.handle_decryption)
        btn_dec.pack(pady=5)

    def browse_config_file(self) -> None:
        """Открывает диалог выбора существующего JSON-файла."""
    ...
    def create_config_file(self) -> None:
        """Создаёт новый JSON-файл с настройками по умолчанию (константы блока)."""
    ...
    def open_config_editor(self) -> None:
        ...
        ttk.Label(
            editor_window,
            text=f"Редактирование файла:\n{self.config_path}",
            justify="left", padding=5, wraplength=420
        ).pack(anchor="w")

        text_area = tk.Text(editor_window, wrap="word", width=50, height=14)
        ...
        def save_json_changes():
            ...
            try:
                ...
                file_io.write_json(self.config_path, parsed_json)
