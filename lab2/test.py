...
def runs_test(seq):
    ...
    # |pi - 0.5| должно быть меньше 2/sqrt(n)
    if abs(pi - 0.5) >= 2.0 / math.sqrt(n):
        return 0.0
    
    # Вычисляем количество знакоперемен (runs)
    runs = 1
    for i in range(n - 1):
        if seq[i] != seq[i + 1]:
            runs += 1
...
def longest_run_test(seq):
    ...
    for i in range(0, n, m):
        ...
        # Распределение по категориям (согласно документации NIST)
        if max_run <= 1:
            v[0] += 1
        elif max_run == 2:
            v[1] += 1
    ...
    # Для 4 категорий степени свободы = 3, поэтому используем gammaincc(3/2, chi2/2)
...
def print_usage():
    ...
    print("   python nist_tests.py sequence_cpp.txt")
    print("\n2. Тестирование нескольких файлов:")
    print("   python nist_tests.py sequence_cpp.txt sequence_java.txt sequence_py.txt")
    print("\n3. Тестирование всех файлов sequence_*.txt в папке:")
    print("   python nist_tests.py all")
    print("\n4. Интерактивный режим (без аргументов):")
    print("   python nist_tests.py")
    print("\n" + "=" * 60)
