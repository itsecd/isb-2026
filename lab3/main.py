...
from crypto.hybrid import secure_clear_key


class CryptoGUI:
    """
    Главный класс графического приложения.
    Содержит все виджеты и обработчики событий.
    """
    ...
    def _create_encrypt_tab(self):
        ...
        ttk.Label(frame, text='Алгоритм шифрования:').grid(
        ...
        ttk.Label(frame, text='Текст для шифрования:').grid(
        ...
        ttk.Button(
            ...
            command=self.load_plaintext_file 
    ...
    def _create_decrypt_tab(self):
        ...
        algo_combo = ttk.Combobox(
            frame,
            textvariable=self.decrypt_algorithm,
            values=['SEED', 'ChaCha20'],
            state='readonly'
        )
        algo_combo.grid(row=0, column=1, sticky='ew', padx=10, pady=5)
        ...
        info_frame = ttk.LabelFrame(
            frame,
            text='Зашифрованные файлы',
            padding=10   
        )
        info_frame.grid(row=1, column=0, columnspan=2, sticky='ew', padx=10, pady=10)
        ttk.Label(info_frame, text='Файл с шифротекстом:').grid(
            row=0, column=0, sticky='w'
        )
        
        self.ciphertext_path = tk.StringVar()
        ttk.Entry(info_frame, textvariable=self.ciphertext_path, width=40).grid(
        ...
        ttk.Label(info_frame, text='Файл с ключом:').grid(
            row=1, column=0, sticky='w'
        )
        
        self.key_file_path = tk.StringVar()
        ttk.Entry(info_frame, textvariable=self.key_file_path, width=40).grid(
        ...
        ttk.Button(frame, text='Расшифровать', command=self.decrypt_data).grid(
            row=2, column=0, columnspan=2, pady=10
        )
        ttk.Label(frame, text='Расшифрованный текст:').grid(
            row=3, column=0, sticky='nw', padx=10, pady=5
        )
        
        self.decrypted_text = scrolledtext.ScrolledText(frame, height=12, width=60)
        self.decrypted_text.grid(
    ...
    def _create_keys_tab(self):
        ...
        ttk.Button(
            rsa_frame,
            text='Сгенерировать новые RSA ключи',
    ...
    def _init_keys(self):
        """Инициализация ключей при запуске приложения."""
        try:
            self.key_manager.ensure_rsa_keys_exist()
        except Exception as e:
            messagebox.showwarning(
                'Инициализация ключей',
                f'Не удалось инициализировать ключи: {e}'
            )
    
    def load_plaintext_file(self):
    ...
    def encrypt_data(self):
        ...
        if key_hex.strip():
            try:
                custom_key = bytes.fromhex(key_hex.strip())
        ...
        def encrypt_thread():
            ...
            try:
                ...
                with self.hybrid_crypto.encrypt_hybrid_secure(
                    plaintext.encode('utf-8'),
                    algorithm,
                    public_key,
                    custom_key
                ) as (encrypted_data, encrypted_key):
                    ...
                    self.root.after(0, lambda: self._show_encrypt_result(
                        encrypted_data, encrypted_key, enc_file, key_file
                    ))
    ...
    def select_ciphertext_file(self):
            """Открывает диалог выбора файла с шифротекстом."""
            file_path = filedialog.askopenfilename(
                title='Выберите файл с шифротекстом',
                filetypes=[
                    ('Все файлы', '*.*'),
                    ('Бинарные файлы', '*.bin'),
                    ('Зашифрованные данные', '*.enc')
                ]
            )
            if file_path:
                self.ciphertext_path.set(file_path)
                print(f"Выбран файл шифротекста: {file_path}")  
                
    def select_key_file(self):
        """Открывает диалог выбора файла с зашифрованным ключом."""
        file_path = filedialog.askopenfilename(
        ...
        if file_path:
            ...
            print(f"Выбран файл ключа: {file_path}")
            
    def _show_encrypt_result(self, encrypted_data: bytes, encrypted_key: bytes,
                             enc_file: str, key_file: str):
        """Отображает результат шифрования в GUI."""
    ...
    def decrypt_data(self):
        """Расшифровывает файлы, выбранные пользователем."""
        
        ciphertext_file = self.ciphertext_path.get()
        key_file = self.key_file_path.get()
        algorithm = self.decrypt_algorithm.get()
        
        if not ciphertext_file or not key_file:
            messagebox.showwarning(
                'Предупреждение',
                'Выберите оба файла: шифротекст и ключ'
            )
            return
        ...
        def decrypt_thread():
            """
            Расшифрование в отдельном потоке.
            Использует контекстный менеджер decrypt_hybrid_secure().
            """
            private_key = None
            ...
            try:
                ...
                with self.hybrid_crypto.decrypt_hybrid_secure(
    ...
    def _show_decrypt_result(self, plaintext: str):
        """
        Отображает расшифрованный текст в GUI.
        ...
        messagebox.showinfo(
            'Успех',
            f'Расшифрование завершено! Получено {len(plaintext)} символов'
        )
        if messagebox.askyesno('Сохранение', 'Сохранить расшифрованный текст в файл?'):
    ...
    def generate_rsa_keys(self):
        ...
        if messagebox.askyesno(
            'Подтверждение',
            'Сгенерировать новые RSA ключи? Старые ключи будут перезаписаны!'
        ):
            try:
    ...
    def refresh_key_info(self):
        """Обновляет отображение информации о ключах."""
