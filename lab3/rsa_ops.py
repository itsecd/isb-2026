...
def generate_rsa_keys(public_exponent: int, key_size: int):
    """
    Генерирует пару RSA ключей.

    :param public_exponent: Публичная экспонента RSA.
    :param key_size: Размер ключа RSA.
    :return: Кортеж из private_key и public_key.
    :raises KeyGenerationError: При ошибке генерации.
    """
    try:
...
def save_private_key(private_key, path: str) -> None:
    ...
    :raises FileProcessingError: При ошибке записи.
    """
    try:
        ...
        write_bytes(path, private_key_bytes)
    except Exception as error:
...
def save_public_key(public_key, path: str) -> None:
    ...
    try:
        ...
        write_bytes(path, public_key_bytes)
    except Exception as error:
...
def load_private_key(path: str):
    ...
    :return: Объект private key.
    :raises KeyLoadError: При ошибке загрузки.
    """
    try:
...
def decrypt_symmetric_key(encrypted_key: bytes, private_key) -> bytes:
    ...
    :param encrypted_key: Зашифрованный ключ.
    :param private_key: Приватный RSA ключ.
