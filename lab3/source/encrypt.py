...
def encrypt_cast5(key: bytes, data: bytes) -> bytes:
    """
    Шифрует данные с помощью CAST5 (CBC).
...
def run(config: Dict[str, Any]) -> None:
    ...
    encrypted = encrypt_cast5(sym_key, data)
...
def parse_args() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Hybrid crypto encryption")
    parser.add_argument(
        "--config",
        required=True,
