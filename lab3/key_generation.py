...
def generate_symmetric_key(key_size:int) -> bytes:
    """
    Generate random key for symmetric algorithm (Camellia)
    Args:
...
def encrypt_symmetric_key(symmetric_key: bytes, public_key: bytes) -> bytes:
    """
    Encrypt the key for symmetric algorithm
    Args:
        public_key(bytes): public RSA key
