...
def main():
    ...
    if config["mode"] == "gen":
        ...
        encrypt_text_bytes = encryption.encrypt_text(text, sym_key)
        encrypt_sym_key = encryption.encrypt_symmetric_key(sym_key, public_key)
        ...
        sym_key = decryption.decrypt_key(sym_key_encrypted, private_key)
        encrypted_text = load_and_save_data.read_binary_file(config["encrypted_file"])
        decrypted_text = decryption.decrypt_text(encrypted_text, sym_key)
