...
def check(P1: float, P2: float, P3: float):
    ...
    if P1 >= 0.01:
        result += f'Частотный побитовый анализ пройден успешно {P1:.4f}\n'
    else:
        result += f'Частотный побитовый анализ не пройден {P1:.4f}\n'

    if P2 >= 0.01:
        result += f'Тест на одинаковые подряд идущие биты пройден успешно {P2:.4f}\n'
    else:
        result += f'Тест на одинаковые подряд идущие биты не пройден {P2:.4f}\n'

    if P3 >= 0.01:
        result += f'Тест на самую длинную последовательность единиц в блоке пройден успешно {P3:.4f}\n'
    else:
        result += f'Тест на самую длинную последовательность единиц в блоке не пройден {P3:.4f}\n'
