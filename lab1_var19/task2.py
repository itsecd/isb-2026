...
def interactive_refine(mapping: Dict[str, str], encrypted_text: str) -> Dict[str, str]:
    ...
    while True:
        ...
        if choice == '1':
            enc_char = input("Введите символ из зашифрованного текста: ").strip()
            if enc_char not in refined:
                print("Такого символа нет в ключе!")
                continue
            
            print(f"Текущее соответствие: {enc_char} -> {refined[enc_char]}")
            new_value = input("Введите новое значение (русскую букву или пробел): ").strip()
...
def decrypt_task2() -> None:
    ...
    print(f"Зашифрованный текст прочитан, длина: {len(encrypted_text)} символов")
