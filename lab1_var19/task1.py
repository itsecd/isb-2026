...
def main() -> None:
    ...
    if args.shift < 0:
        print(
            f"Предупреждение: отрицательный сдвиг {args.shift} будет преобразован в положительный")
        args.shift = args.shift % alphabet_length

    if args.shift >= alphabet_length:
        print(
            f"Предупреждение: сдвиг {args.shift} больше длины алфавита ({alphabet_length})")
