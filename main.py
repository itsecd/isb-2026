...
def get_args():
    ...
    parser.add_argument('--key_len', type=int, choices=[128, 192, 256], default=256, 
                        help='Длина ключа AES в битах (128, 192 или 256). По умолчанию 256')
...
def get_settings() -> dict:
    ...
    final_config = {
        ...
        "initial_file": args.init or config.get("initial_file", "text.txt"),
    ...
    match config["mode"]:
        ...
        case "enc":
            ...
            if public_key is None:
                print("[-] Ошибка: не удалось загрузить открытый ключ")
                return
        ...
        case "dec":
            ...
            if private_key is None:
                print("[-] Ошибка: не удалось загрузить закрытый ключ")
                return
            
            sym_key_encrypted = load_and_save_data.load_encrypt_symmetric_key(config["symmetric_key"])
            if not sym_key_encrypted:
                print("[-] Ошибка: зашифрованный ключ не найден или пуст")
                return
            ...
            if not encrypted_text:
                print("[-] Ошибка: зашифрованный файл не найден или пуст")
                return
