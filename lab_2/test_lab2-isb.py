...
def load_sequence(filename):
    ...
    Args:
        filename (str): Путь к файлу, содержащему битовую последовательность.
    
    Returns:
        str: Строка, содержащая биты (символы '0' и '1').
...
def monobit_test(seq):
    ...
    Проверяет, не слишком ли количество единиц отличается от количества нулей.
    Случайная последовательность должна иметь примерно равное количество 0 и 1.
    
    Args:
        seq (str): Битовая последовательность из символов '0' и '1'.
    
    Returns:
        tuple: (bool, float) - (пройден ли тест, p-value)
    """
    n = len(seq)
...
def runs_test(seq):
    ...
    Проверяет, соответствует ли количество серий (непрерывных блоков
    одинаковых битов) ожидаемому для случайной последовательности.
...
def longest_run_test(seq):
    ...
    Разбивает последовательность на блоки по 8 бит и проверяет распределение
    максимальных длин серий единиц в каждом блоке. Использует критерий хи-квадрат.
...
def test_file(filename):
    ...
    Загружает последовательность из файла, проверяет её валидность
    (только символы '0' и '1', минимальная длина 100 бит) и выполняет
    все статистические тесты.
    
    Args:
        filename (str): Путь к файлу с битовой последовательностью.
    
    Returns:
        list: Список кортежей вида [(название_теста, (пройден, p-value)), ...]
    
    Raises:
        ValueError: Если файл содержит недопустимые символы или последовательность слишком короткая.
        FileNotFoundError: Если файл не существует.
    """
...
def save_results_to_file(all_results, output_filename):
    ...
    Формирует подробный отчёт о тестировании, включающий результаты
    каждого файла, количество пройденных тестов и общую статистику.
    
    Args:
        all_results (list): Список кортежей [(filename, tests), ...],
                           где tests - результат вызова test_file().
        output_filename (str): Путь к файлу для сохранения отчёта.
    
    Returns:
        str: Путь к сохранённому файлу отчёта.
    """
...
def main():
    ...
    Перебирает все файлы из списка fp.TEST_FILES, запускает для каждого
    тестирование, выводит промежуточные результаты в консоль и сохраняет
    полный отчёт в файл fp.OUTPUT_FILE.
