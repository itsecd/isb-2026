...
def generate_cpp_sequences():
    ...
    subprocess.run(['g++', '-o', './random_cpp', 'gen_cpp.cpp', '-std=c++11'])
...
def generate_java_sequences():
    ...
    subprocess.run(['javac', 'GenJava.java'])
    subprocess.run(['java', 'GenJava'])
    print()
...
def main():
    ...
    with open('final_results.txt', 'w', encoding='utf-8') as f:
        f.write("РЕЗУЛЬТАТЫ ЛАБЫ №2\n")
        
        f.write("СГЕНЕРИРОВАННЫЕ ПОСЛЕДОВАТЕЛЬНОСТИ\n")
        
        for name, seq in sequences.items():
            f.write(f"{name}:\n{seq}\n\n")
        
        f.write("\nРЕЗУЛЬТАТЫ ТЕСТИРОВАНИЯ NIST\n")
