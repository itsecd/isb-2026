...
def longest_run_in_block_prepare(seq: str) -> Tuple[List[int], float, int, float, float]:
    """Считает параметры Longest Run (N=128,M=8): v, chi2, K, a=K/2, x=chi2/2 (для калькулятора)."""
    n: int = len(seq)
    if n != 128:
        raise ValueError("Этот вариант теста рассчитан на N=128 (как в методичке).")
    ...
    # Вероятности π_i для M=8 (константы из методички)
...
def parse_p_value(user_input: str) -> float:
    """Преобразует введённое пользователем p-value в float (поддерживает запятую)."""
