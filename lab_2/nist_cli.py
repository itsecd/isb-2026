...
def main() -> None:
    ...
    print(f"Читаю последовательность из файла: {args.seq_file}")

    ensure_dir(args.out_dir)
    seq: str = read_bitstring(args.seq_file, expected_len=128)

    # Monobit
    print("\n[1/3] Частотный побитовый тест (Monobit)...")
    ...
    print(f"Готово. P-value = {p_mono:.6f}.")
    print(f"Файл: {os.path.join(args.out_dir, 'monobit_result.txt')}")
    ...
    if not condition_ok:
        ...
        print("Условие |pi - 1/2| < 2 / sqrt(N) не выполнено.")
        print("P-value = 0.000000")
    else:
        runs_text += (
            "Vn = Σ r_i, где r_i = 1, если ε_i != ε_(i+1), иначе 0\n"
            f"Vn = {vn}\n"
            f"P-value = erfc(|Vn - 2(N-1)pi(1-pi)| / (2*sqrt(2(N-1))*pi(1-pi))) = {p_runs:.10f}\n"
    ...
    print("Посчитайте P-value = igamc(a, x) и введите его в консоль.")

    while True:
        try:
            p_in: str = input("Введите P-value из калькулятора (0..1): ")
    ...
    long_text: str = (
        "NIST Longest Run of Ones in a Block (N=128, M=8)\n"
        f"Файл: {args.seq_file}\n"
    ...
    print(f"Готово. P-value = {p_long:.6f}.")
    print(f"Файл: {os.path.join(args.out_dir, 'longest_run_result.txt')}")
    ...
    summary: str = (
        ...
        f"alpha = {args.alpha}\n\n"
        f"Monobit:  P={p_mono:.10f}  -> {'ПРОЙДЕН' if mono_pass else 'НЕ ПРОЙДЕН'}\n"
        f"Runs:     P={p_runs:.10f}  -> {'ПРОЙДЕН' if runs_pass else 'НЕ ПРОЙДЕН'}\n"
        f"Longest:  P={p_long:.10f}  -> {'ПРОЙДЕН' if long_pass else 'НЕ ПРОЙДЕН'}\n"
    )
